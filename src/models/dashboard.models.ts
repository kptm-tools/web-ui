export interface SecurityPosture {
  score: number;
  variation: number;
}

export interface SeverityCount {
  critical: number;
  high: number;
  medium: number;
  low: number;
}

export type SeverityCountLevels = 'critical' | 'high' | 'medium' | 'low';

export interface HostSeverity {
  alias: string;
  severity_count: SeverityCount;
}

export interface VulnerabilityTrend {
  time_period: string;
  vulnerability_count: number;
}

export interface LastScan {
  host_alias: string;
  total_vulnerabilities: number;
  total_vulnerabilities_variation: number;
  severity_counts: SeverityCount;
  scan_date: string; // ISO date string
}

export interface HostVulnerability {
  alias: string;
  vulnerability_count: number;
  color?: string;
}

export interface iMainDashboard {
  overall_security_posture: SecurityPosture;
  host_severity_heat_map: HostSeverity[];
  vulnerability_trends: VulnerabilityTrend[];
  last_scan: LastScan | null;
  hosts_with_greatest_vulnerabilities: HostVulnerability[];
}

export interface iSerieDashboard {
  name: string;
  data: (number | string)[];
}
