<template>
  <q-card-section>
    <div class="row">
      <q-select
        v-model="pickedHost"
        outlined
        dense
        :options="hostPickedOptions"
        option-label="domain"
        class="col-6"
      />
    </div>
    <template v-for="host in hostPickedOptions" :key="host">
      <template v-if="host.domain === pickedHost.domain">
        <div class="row">
          <div class="text-weight-bold q-mt-md">Start Scan</div>
        </div>

        <div class="row q-col-gutter-md items-center flex full-width">
          <q-radio
            v-model="shouldStartScanNow"
            name="shape"
            :val="true"
            label="Right now"
            class="col-4"
            @update:model-value="
              value => {
                if (value) {
                  host.scanDateTime.date = null;
                  host.scanDateTime.time = null;
                }
              }
            "
          />
        </div>
        <div class="row q-col-gutter-md items-center flex full-width">
          <q-radio
            v-model="shouldStartScanNow"
            name="shape"
            :val="false"
            label="Schedule for"
            class="col-4"
          />
          <q-input
            v-model="host.scanDateTime.date"
            outlined
            dense
            mask="date"
            :disable="shouldStartScanNow"
            :rules="['date']"
            class="col-4"
          >
            <template #append>
              <q-icon name="event" class="cursor-pointer" :disable="shouldStartScanNow">
                <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                  <q-date
                    v-model="host.scanDateTime.date"
                    @update:model-value="handleAllSelectDate"
                  >
                    <div class="row items-center justify-end">
                      <q-btn v-close-popup label="Close" color="primary" flat />
                    </div>
                  </q-date>
                </q-popup-proxy>
              </q-icon>
            </template>
          </q-input>

          <q-input
            v-model="host.scanDateTime.time"
            outlined
            dense
            mask="time"
            :rules="['time']"
            :disable="shouldStartScanNow"
            class="col-4"
          >
            <template #append>
              <q-icon name="access_time" class="cursor-pointer">
                <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                  <q-time
                    v-model="host.scanDateTime.time"
                    :disable="shouldStartScanNow"
                    @update:model-value="handleAllSelectTime"
                  >
                    <div class="row items-center justify-end">
                      <q-btn v-close-popup label="Close" color="primary" flat />
                    </div>
                  </q-time>
                </q-popup-proxy>
              </q-icon>
            </template>
          </q-input>
        </div>

        <div class="row">
          <div class="text-weight-bold q-mt-md">Scheduled Scan</div>
        </div>
        <div class="row q-col-gutter-md items-center flex full-width">
          <q-radio
            v-model="shouldRunOnce"
            name="shape"
            :val="true"
            label="Run once"
            class="col-4"
            @update:model-value="
              value => {
                if (value) {
                  host.repeat_frequency.quantity = 1;
                  host.repeat_frequency.unit_of_frequency = 'Day';
                }
              }
            "
          />
        </div>
        <div class="row q-col-gutter-md items-center flex q-mb-md full-width">
          <q-radio
            v-model="shouldRunOnce"
            name="shape"
            :val="false"
            label="Repeat every"
            class="col-4"
          />

          <q-input
            v-model.number="host.repeat_frequency.quantity"
            type="number"
            outlined
            dense
            style="max-width: 200px"
            :disable="shouldRunOnce"
            class="col-4"
          />

          <q-select
            v-model="host.repeat_frequency.unit_of_frequency"
            outlined
            dense
            :options="timePeriodOptions"
            :disable="shouldRunOnce"
            class="col-4"
          />
        </div>
      </template>
    </template>
  </q-card-section>

  <q-card-actions align="right">
    <q-btn type="submit" label="Next" color="primary" size="md" @click="submitTime" />
  </q-card-actions>
</template>

<script setup lang="ts">
  import type { Host, HostSchedule } from 'src/models';
  import type { Ref } from 'vue';
  import { ref, watch } from 'vue';

  const emit = defineEmits(['submitTime']);

  const props = defineProps({
    hosts: {
      type: Array as () => Host[],
      required: true
    }
  });

  const allOptionHost = {
    credentials: [],
    rapporteurs: [],
    domain: 'All',
    hostName: 'All'
  };
  const pickedHost: Ref<Host> = ref(allOptionHost as Host);
  const shouldStartScanNow = ref(true);
  const shouldRunOnce = ref(true);
  const timePeriodOptions = ['day', 'week', 'month', 'year'];

  const hostPickedOptions: Ref<HostSchedule[]> = ref([]);

  watch(
    props.hosts,
    () => {
      if (props.hosts.length) {
        const hostsAux: Host[] = [...props.hosts];
        hostsAux.unshift(allOptionHost);
        hostPickedOptions.value = hostsAux.map(val => ({
          ...val,
          scanDateTime: { date: null, time: null },
          repeat_frequency: { quantity: 1, unit_of_frequency: 'day' }
        })) as HostSchedule[];
      }
    },
    { immediate: true }
  );
  function submitTime(): void {
    emit('submitTime', hostPickedOptions.value);
  }

  function handleAllSelectDate(date: string): void {
    if (pickedHost.value.domain === 'All') {
      hostPickedOptions.value = hostPickedOptions.value.map(val => ({
        ...val,
        scanDateTime: { date, time: val.scanDateTime.time }
      }));
    }
  }

  function handleAllSelectTime(time: string | null): void {
    if (pickedHost.value.domain === 'All') {
      hostPickedOptions.value = hostPickedOptions.value.map(val => ({
        ...val,
        scanDateTime: { time, date: val.scanDateTime.date }
      }));
    }
  }
</script>
