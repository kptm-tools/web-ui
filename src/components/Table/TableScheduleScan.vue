<template>
  <table-regular
    :columns="SCAN_SCHEDULE_TABLE_COLUMNS"
    :rows="scheduleScans"
    :show-actions="false"
    @action="handlerEmitter($event)"
  >
    <template #column="{ column, row }">
      <template v-if="column.field === 'id'"> </template>
      <template v-if="column.field === 'scheduled_date'">
        {{ formatTableDate(column.value) }}</template
      >
      <template v-else-if="column.field === 'actions'">
        <template v-for="action in updatedTableActions" :key="action.name">
          <q-btn
            v-if="action.show(row.status)"
            :icon="action.icon"
            class="button-table-action"
            dense
            flat
            @click="() => handlerEmitter({ action, row })"
          />
        </template>
      </template>

      <template v-else>
        {{ column.value }}
      </template>
    </template>
  </table-regular>
</template>

<script setup lang="ts">
  import type { ComputedRef } from 'vue';
  import { computed, type PropType } from 'vue';
  import { TableRegular } from 'src/components';
  import { SCAN_SCHEDULE_TABLE_COLUMNS } from 'src/constants/table.constants';
  import { formatTableDate } from 'src/utils';
  import { denyActionsStore } from 'src/stores/deny-actions-store';
  import { DENY_ACTIONS } from 'src/constants/deny-actions.constants';
  import type { ScanTableAction } from 'src/models';
  import { ScanStatus, ScanActions } from 'src/models';

  defineProps({
    scheduleScans: {
      type: Array as PropType<Record<string, unknown>[]>,
      default: () => []
    }
  });

  const { isAbleToHandleAction } = denyActionsStore();

  const emits = defineEmits(['action']);
  const updatedTableActions: ComputedRef<ScanTableAction[]> = computed(() => {
    const actions: ScanTableAction[] = [];
    const addEditSchedule = isAbleToHandleAction(DENY_ACTIONS.SCAN_SCHEDULE_PATCH_BY_ID);
    const addDeleteSchedule = isAbleToHandleAction(DENY_ACTIONS.SCAN_SCHEDULE_DELETE_BY_ID);

    if (addEditSchedule) {
      actions.push({
        name: ScanActions.insight,
        icon: 'fas fa-chart-simple',
        show: (value: ScanStatus): boolean => value === ScanStatus.completed
      });
    }
    if (addDeleteSchedule) {
      actions.push({
        name: ScanActions.cancel,
        icon: 'close',
        show: (value: ScanStatus): boolean =>
          value === ScanStatus.inProgress || value === ScanStatus.pending
      });
    }
    return actions;
  });

  function handlerEmitter(action: unknown): void {
    emits('action', action);
  }
</script>
