  ...
  <q-form @submit="validateHostHandler">
    <div class="row">
      ...
      <div class="col-9 flex justify-between">
        <q-input v-model="domain" outlined dense :disable="!isDomain" class="input-text" />
        <q-btn
          square
          color="primary"
          icon="add"
    ...
    <div class="row q-mb-md">
      <template v-for="validatedDomain in validatedDomains" :key="validatedDomain.value">
        <q-chip
          clickable
          color="grey"
          text-color="white"
    ...
    <div class="row">
      ...
      <div class="col-9 flex justify-between">
        <q-input v-model="ip" outlined dense :disable="isDomain" class="input-text" />
        <q-btn
          square
          color="primary"
          icon="add"
    ...
    <div class="row q-mb-md">
      <q-input v-model="hostname" outlined dense>
        <template #before> <label class="input-label">Hostname</label></template></q-input
      >
    </div>

    <div class="row justify-center">
      <q-btn v-if="validateIsEnable" color="primary" label="Validation" type="submit" />
...
<script setup lang="ts">
  ...
  const hostnameValidates = ref([] as { value: string; hostname: string; isDomain: boolean }[]);

  const validateIsEnable = computed(
    () => (domain.value.length > 0 || ip.value.length > 0) && hostname.value.length > 0
  );

  const validatedDomains = computed(() => hostnameValidates.value.filter(host => host.isDomain));

  const validatedIps = computed(() => hostnameValidates.value.filter(host => !host.isDomain));
  ...
  function addHostValidated(value: string, hostname: string, isDomain: boolean) {
  ...
  function removeHostValidated(value: string) {
    hostnameValidates.value = hostnameValidates.value.filter(host => host.value !== value);
  }
