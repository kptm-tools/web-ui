<template>
  <q-tabs v-model="tab" class="text-teal q-mb-md" content-class="justify-start text-primary">
    <q-tab name="scans" label="Scans" />
    <q-tab name="scheduled" label="Scheduled Scans" v-if="showScheduledScans" />
  </q-tabs>
  <q-btn label="New Scan" color="primary" class="q-mb-md" @click="newScan"></q-btn>
  <template v-if="tab === TAB_OPTIONS.SCANS">
    <table-scan :scans="rows" @action="handlerEmitter" />
  </template>
  <template v-if="tab === TAB_OPTIONS.SCHEDULED">
    <table-schedule-scan :schedule-scans="scheduleScan" @action="handlerEmitter" />
  </template>
</template>

<script setup lang="ts">
  import { computed, onMounted, ref, watch, type Ref } from 'vue';
  import { useQuasar } from 'quasar';
  import { HostService, ScanService, ScanSchedulesService } from 'src/services';
  import type { CreateScanBody, Host, HostSchedule } from 'src/models';
  import { DialogScan, TableScheduleScan, TableScan } from 'src/components';
  import { formatHostScheduleToCreateScanBody } from 'src/utils';
  import { denyActionsStore } from 'src/stores/deny-actions-store';
  import { DENY_ACTIONS } from 'src/constants/deny-actions.constants';

  const { isAbleToHandleAction } = denyActionsStore();

  enum TAB_OPTIONS {
    SCANS = 'scans',
    SCHEDULED = 'scheduled'
  }

  defineProps<{
    rows: Record<string, unknown>[];
  }>();

  const emits = defineEmits(['refreshTable', 'action']);
  const $q = useQuasar();
  const hosts: Ref<Host[]> = ref([]);
  const tab: Ref<TAB_OPTIONS> = ref(TAB_OPTIONS.SCANS);
  const scheduleScan = ref([]);

  const showScheduledScans = computed(() =>
    isAbleToHandleAction(DENY_ACTIONS.SCAN_SCHEDULE_GET_ALL)
  );

  function newScan(): void {
    $q.dialog({
      component: DialogScan,
      componentProps: {
        hosts: hosts.value
      }
    }).onOk((data: HostSchedule[]) => {
      const formattedCreateScanBody: CreateScanBody[] = formatHostScheduleToCreateScanBody(data);
      formattedCreateScanBody.forEach(value => {
        ScanService.createScan(value).catch(err => new Error(err));
      });
    });
  }

  function handlerEmitter(action: unknown): void {
    emits('action', action);
  }

  watch(tab, () => {
    if (tab.value === TAB_OPTIONS.SCHEDULED) {
      ScanSchedulesService.getScanSchedules()
        .then(res => {
          scheduleScan.value = res.data;
        })
        .catch(err => new Error(err));
    }
  });

  onMounted(async () => {
    hosts.value = (await HostService.getHosts()).data;
  });
</script>

<style scoped></style>
