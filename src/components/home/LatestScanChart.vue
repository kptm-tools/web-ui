<template>
  <div class="row q-mb-md">
    <div class="col-12">
      ...
      <div class="subtitle" style="color: #5c7288">
        ...
        {{ dashboard.last_scan?.scan_date.slice(0, 10) || '' }}
      </div>
    </div>
  ...
  <div class="row flex items-center q-mb-md protection">
    <div class="col-6 flex items-center justify-center text-weight-bold protection-score">
      {{ dashboard?.last_scan?.total_vulnerabilities }}
    </div>
    <div class="col-6">
      <p class="q-ma-none protection-host">
        {{ dashboard?.last_scan?.host_alias }}
      </p>
      <p class="q-ma-none protection-variation">
        {{ dashboard?.last_scan?.total_vulnerabilities_variation }}
...
<script lang="ts" setup>
  ...
  import { MainDashboard } from 'src/services/dashboard';
  ...
  const donutSeverityCountsSeries = computed(
    () => new MainDashboard(props.dashboard).donutSeverityCountsSeries
  );

  const variationIcon: ComputedRef<string> = computed(() =>
    getVariationIcon(props.dashboard.overall_security_posture?.variation || 0)
  );
