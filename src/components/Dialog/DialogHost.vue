<template>
  <q-dialog ref="dialogRef" @hide="onDialogHide">
    <q-card class="q-dialog-plugin full-width" style="max-width: 700px; width: 100%">
      ...
      <q-card-section>
        <form-validate-host v-if="step == 'validate'" :hosts="hosts" @validated-hosts="hostValidationHandler" />
        <form-host-credentials v-if="step == 'auth'" :edit="false" :hosts="validatedHosts"
          @host-credentials="hostCredentialsHandler" />

        <form-host-emails v-if="step == 'email'" :hosts="validateHostsAuth" @register-host="hostCreationHandler" />
      </q-card-section>
