  ...
  <q-dialog ref="dialogRef" class="dialog-scan-insight" @hide="onDialogHide">
    <q-card class="q-dialog-plugin full-width">
      <q-card-section class="q-mb-md row q-col-gutter-sm">
        ...
        <div class="col-3">
          <q-select v-model="severityPicked" outlined multiple dense :options="severityOptions" label="Severity"
            @update:model-value="handlerSelect" />
        </div>
        <div class="col-3">
          <q-select v-model="timePicked" outlined dense :options="timeOptions" label="Time Range"
            @update:model-value="handlerSelect" />
        </div>
      </q-card-section>
      <q-card-section>
          ...
          <div class="col-6">
            <div class="row q-mb-md">
              ...
              <div class="full-width">
                <apexchart :options="SCAN_INSIGHT_VULNERABILITY_OPTIONS" :series="generalSummaryValues"></apexchart>
              </div>
            </div>
          </div>
          <div v-if="showGraph" class="col-6">
            <div class="title">Vulnerabilities by Category</div>
            <apexchart :options="vulnerabiltyOptions" :series="vulnerabilitiesValues" :height="300"></apexchart>
          </div>
        </div>
        <div class="row justify-center">
          <apexchart type="line" height="300" :options="vulnerabilityTrendOptions" :series="vulnerabilitiesTrendValues"
            width="700"></apexchart>
        </div>
...
async function handlerSelect(): Promise<void> {
  showGraph.value = false;
  await ReportService.getReportsVulnerabilitiesSummary(
